...
uint256 hashGenesisBlock("0xc8a91e81ed2ed2c4113f1c4dd8197fb9841cdac1174788d5da5db3d969c4c6cb");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Deftcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Deftcoin Signed Message:\n";
...
{
    // Deftcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 1051200); // Deftcoin: 840k blocks in ~2 years
...
static const int64 nTargetTimespan = 24 * 60 * 60; // Deftcoin: 1 days
static const int64 nTargetSpacing = 1 * 60; // Deftcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Deftcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
        ...
        if (fTestNet) {
                if (pindexLast->nHeight+1 >= 2116) { DiffMode = 2; } // deftcoin, 100 blocks after first legacy diff adjustment
        }
        else {         
        	if (pindexLast->nHeight+1 >= 26754) { DiffMode = 2; }  //deftcoin, 5 days after 27/01/2014 12:00 UTC
        }
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Deftcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Deftcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nNonce = 7050090;
        block.nTime = 1394252668;
        
        if (fTestNet)
        {

            block.nNonce = 11521194;
            block.nTime = 1394252385;
        }
	...
	/*int i = 0;
	int nnonce = block.nNonce;
	printf("Huh");
        while (!CheckProofOfWork(hash,block.nBits)) {
		block.nNonce++;
                hash = block.GetHash();
		i++;
		if (i%10000 == 0) {
...
unsigned char pchMessageStart[4] = { 0xfa, 0xbf, 0xb5, 0xda }; // Deftcoin: increase each by adding 1 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DeftcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DeftcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DeftcoinMiner(CWallet *pwallet)
{
    printf("DeftcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("deftcoin-miner");
    ...
    try { loop {
        ...
        printf("Running DeftcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DeftcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DeftcoinMiner, pwallet));
}
